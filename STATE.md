...
|-------|----------|----------|
| Global | `~/.config/alph/config.yaml` | Creator email, auto_commit, default registry, default pool, registry declarations (including mode, clone_path, auto_push, branch for remote registries) |
...
| `alph registry status <id>` | | Show registry mode, clone state, branch, auto_push, and path details |
...
- Auto-commit on add (opt-in via config); `auto_push` for RW remote registries (opt-in). No auto-pull
...
Phase 1, Phase 2, and remote registry support are complete. The project is at v0.1.16 (Homebrew).
...
237 tests passing. Full TDD — every production function written test-first. mypy strict clean, ruff clean.

### Distribution

- **Homebrew tap**: `AlpheusCEF/homebrew-tap`, formula at v0.1.16. `brew tap AlpheusCEF/tap && brew install alph` installs both `alph` and `alph-mcp` binaries. Formula uses `preserve_rpath` to avoid Rust-extension dylib relocation issues.
